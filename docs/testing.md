...
# Override the insert/update/delete sample size when needed.
dotnet run --project benchmarks/SlimVector.Benchmarks -c Release -- --e2e --profile Smoke --operation-count 500
```

The runner measures all vector kinds and quantizations against exact recall, migration build, a real SlimVector HTTP process with vector/text/hybrid traffic plus concurrent reads/writes, controlled backpressure and rate-limit saturation, and a real four-node DotNext add/catch-up path. Every vector index is measured phase by phase for `IndexBuild`, `SelectVector`, `Insert`, `Update`, `Delete`, `PersistSnapshot`, and `ColdLoad`; the HTTP process has equivalent create/insert/select/update/delete phases. Each phase records item and sample counts, wall time, throughput, p50/p95/p99, CPU time and average/peak utilization, working-set before/after/average/peak, managed-memory delta, allocated bytes, Gen0/1/2, LOH, GC pauses, disk delta, and errors.

Every versioned run writes `benchmark-summary.md`, comparative HTML charts, scenario-level `benchmark-results.csv`, phase-level `benchmark-operations.csv`, nested JSON, and environment/version/configuration metadata under `artifacts/benchmarks`. Missing profiles are not synthesized; only an executed profile produces a report. A supplied baseline adds scenario and per-operation regression percentages.
