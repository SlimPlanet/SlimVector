...
> [Documentation index](README.md) · [User guide](user-guide.md) · [Configuration](configuration.md) · [Security](security.md)
...
Start one supplied joiner with a unique node id, zone, disk capacity, internal endpoint and persistent Raft port range. Registration does not move data.

```bash
export SLIMVECTOR_ADMIN_KEY='replace-with-a-secret-at-least-32-characters'
docker compose -f compose.cluster.yml --profile joiner up --build -d node-4-joiner
```
...
curl -fsS -X POST http://localhost:8081/api/v1/admin/cluster/nodes/join \
  ...
  -d '{"nodeId":"node-4","apiEndpoint":"http://localhost:8084","internalEndpoint":"http://172.28.0.14:8080","raftHost":"172.28.0.14","zone":"zone-a","capacityBytes":1099511627776,"raftPortStart":3263,"raftPortCount":128,"roles":["api","data"]}'
```

Those values intentionally match `node-4-joiner` in Compose. In a real deployment, `apiEndpoint` must be a stable address usable for client redirects, while `internalEndpoint` and `raftHost` must be reachable from every cluster node. Use HTTPS/mTLS or an equivalently protected private network in production.
...
To exercise the full 3→6 capacity path, start `node-5-joiner` and `node-6-joiner`, register each with the values from Compose, then generate and approve a new plan after every topology change. A running joiner is not part of placement until its registration succeeds. Useful capacity increases only after the approved replica movements complete.

## Drain and remove

A draining node receives no new replicas. Mark it, generate a plan with `drainNodeId`, inspect and approve that plan, wait until topology reports no replica on that node, then delete it:
...
Do not issue the `DELETE` until the topology contains no data-group replica for `node-4`; removal correctly fails while data remains assigned.
...
1. Confirm `/health/ready` and scrape topology/membership on all nodes.
2. Ensure the target node is not hosting the only reachable quorum path for any group.
3. Drain or transfer leadership when required, then stop only one voter at a time.
4. Start the node with the same stable identity and its own unchanged volume.
5. Wait for transport health and replication lag to recover before continuing.

Capacity rebalancing remains manually approved by default; RF repair after the configured failure delay is automatic when enough active nodes remain. See [observability](observability.md) for the metrics and [troubleshooting](troubleshooting.md) for quorum and movement failures.
